...
<html lang="pt-br">
  ...
  <body>
    ...
    <div class="container">
      ...
      <div class="row justify-content-md-center">
        ...
        <div class="col-8">
          <div class="row">
                <div class="col-5">
                    <img src="../shopping/App/View/images/<?=$product->getFoto();?>" class="img-thumbnail rounded">
                </div>
            <div class="col-7 jumbotron bg-light">
                <?php if(!isset($_SESSION["admin"])){ ?>
                <p class="font-weight-bold"><?= $product->getNome();?></p>
                <p class="font-weight-bold">Publicadora: <a href="?page=games&action=publisher&pub=<?= $product->getPublicadora(); ?>"><?=$product->getPublicadora();?></a></p>
                <p class="font-weight-bold">Preço: R$<?= number_format($product->getPreco(), 2, ',', '.');?></p>
                <p class="font-weight-bold">Estoque: <?=$product->getQuantidade();?></p>
              <p class="text-justify"><?= $product->getDescricao();?></p>

            <form action="index.php?page=cart&action=add" method="post">
              <input type="hidden" name="id" value="<?= $product->getId(); ?>"/>
              <input type="hidden" name="tabela" value="jogos"/> 
              <?php if($product->getQuantidade() > 0){ ?>
              <button type="submit" class="btn btn-primary"><span>Adicionar <img src="../shopping/App/View/images/add.png" width="20px" height="20px"></span></button>
            </form>
            <?php }else{ ?>
              <div class="alert alert-warning">PRODUTO ESGOTADO</div>
              <?php } ?>
              
                <?php }else{ ?>
              <form id="form-update" method="post" action="?page=games&action=update&id=<?=$product->getId();?>" class="form-horizontal">
                  <label for="tPreco">Preço: </label><input type="text" id="tPreco" name="preco" value="<?=$product->getPreco();?>" class="form-control"/> <br/>
                  <label for="tDescricao">Descrição(Opcional): </label> <textarea class="form-control" name="descricao" id="prod_description" maxlength="200" placeholder="Insira uma descrição"><?=$product->getDescricao();?></textarea> <br/>
                  Quantidade: <input min="1" type="number" name="quantidade" class="form-control" value="<?=$product->getQuantidade();?>"/> <br/>
            ...
            <h2><span><img src="../shopping/App/View/images/welcome.png" width="50px" height="50px"/></span> Baseados no Produto Visualizado: </h2>
          <div class="row">
    ...
    <script type="text/javascript" src="../shopping/App/View/javascript/validacao-update-produtos.js"></script>
    <?php endif; ?>
  </body>
</html>